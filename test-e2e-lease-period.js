...
// Area share × $100,000. NOT apportioned by time — that is the point.
const EXPECTED_ALLOCATION = {
  'Alder Bakery':   '$24,000.00',
  'Birch Optical':  '$20,000.00',   // 20% of the pool, not 9/12 of 20%
  'Cedar Fitness':  '$26,000.00',   // 26% of the pool, not 9/12 of 26%
  'Dogwood Deli':   '$14,000.00',
  'Elm Stationers': '$16,000.00',
};
...
(async () => {
  ...
  yes('every allocation is the tenant\'s AREA share of the pool',
  ...
  yes('    the mid-period tenants are billed the FULL period, un-apportioned',
      run.alloc['Birch Optical'] === '$20,000.00' && run.alloc['Cedar Fitness'] === '$26,000.00',
      'an apportioned figure appeared — T1 must not introduce proration');
  yes('    and the billed total still equals the pool',
      run.billed === 100000, String(run.billed));
  ...
  yes('it says the lease ends inside the period',
      !!birch && /lease ends inside the 2026 CAM period/.test(birch), String(birch));
  ...
  yes('it says the lease begins inside the period',
      !!cedar && /lease begins inside the 2026 CAM period/.test(cedar), String(cedar));
  ...
  const stmts = await page.evaluate(async () => {
    ...
    for (const n of ['Alder Bakery', 'Elm Stationers', 'Birch Optical', 'Cedar Fitness', 'Dogwood Deli']) {
      ...
      out[n] = {
        ...
        saysConfirm: /Confirm CAM treatment/.test(txt),
        prorated: /prorat/i.test(txt),
      };
    }
    const o = document.getElementById('reportOverlay'); if (o) o.style.display = 'none';
    return out;
  ...
  yes('the two full-period tenants are billed their area share',
      stmts['Alder Bakery'].issued && stmts['Alder Bakery'].due === '$24,000.00'
        && stmts['Elm Stationers'].issued && stmts['Elm Stationers'].due === '$16,000.00',
  ...
  yes('Birch\'s refusal names the confirmation, not an expiry that has not happened',
      stmts['Birch Optical'].saysConfirm && !stmts['Birch Optical'].saysEnded,
      JSON.stringify(stmts['Birch Optical']));
  yes('no statement anywhere recommends proration',
