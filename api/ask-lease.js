...
if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
  throw new Error('[api/ask-lease] SUPABASE_URL and SUPABASE_ANON_KEY env vars are required');
}
...
// Extracts a JSON { answer, citations } from Claude's response text.
// If parsing fails for any reason the raw text becomes the answer with no citations,
// so the caller always gets a usable string — never a broken state.
...
- Do not include any text outside the JSON object.`;
  ...
  const { answer, citations } = parseStructuredResponse(raw);
