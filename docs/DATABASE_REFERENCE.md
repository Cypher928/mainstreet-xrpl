...
| 027 | `abstraction_error` on `acquisition_documents`, with two checks | WHY a term reading failed, when one did — one of `no_text`, `transport`, `upstream_timeout`, `upstream_error`, `unparsable`, `no_fields`, or null (P4-3 remediation, Issue A4). Added because a live document came back `failed` with nothing on the row to say which of three code paths wrote it, and the real cause turned out to be a client-side abort of a request the server had already answered correctly. **Not** a sixth `abstraction_status`, and **not** `error_message`, which belongs to the parsing stage. Constrained to the six words, and only on a `failed` row, so a reading that later lands cannot keep a stale reason. Pilot only; rollback in `027_..._rollback.sql`; executed end-to-end by `tools/verify-migration-027.js`. **Not yet applied to Pilot.** |
