...
- **The human correction path (P4-3 remediation, Issue B):** `proposeFamily` is the AI's and is UNCHANGED — it may start a leasehold only from an `original_lease`. Alongside it: `familyForCorrection(doc, families, opts)` for the path a person takes, which may begin a leasehold from an `original_lease` correction or from `opts.beginLeasehold` (one control, clicked by a person, never a classification), files a single tenant match as a PROPOSAL, and otherwise returns `none` with a readable reason plus `canBegin` on the one case a person may settle. `tenantHintFor` / `suiteHintFor` read only this document's terms evidence then the tenant it produced — never the file name. `unfiledSiblings(doc, documents)` finds same-tenant, current, lease-family, unfiled rows to be OFFERED a new leasehold, one confirmation each.
- **Tests:** `test-acquisition-documents.js`, `test-e2e-acquisition-documents.js`, `tools/acquisition-documents-mutation.js`, `tools/verify-migration-023.js`, `test-acquisition-classification.js`, `test-e2e-acquisition-classification.js`, `tools/acquisition-classification-mutation.js`, `tools/verify-migration-024.js`, `test-acquisition-terms.js`, `test-acquisition-family-lifecycle.js`.
...
- **Pinned:** by sha256 in `test-acquisition-report-view.js` and `test-acquisition-report-q1q2.js`. Frozen at edcf259 and unfrozen once (§4l): a verified term with `support: 'entered'` projects as `origin: 'entered'` with no evidence, and is counted in `verified_entered`.
...
- **§4l:** a verified fact with `origin: 'entered'` is drawn with the Verified chip and, beside it, the whole sentence "Entered by a person · No document on file supports this value"; the legend explains the mark. Every other fact draws as R-2 drew it.

## acquisition-leasehold.js — one tenant row per leasehold (Phase 1, §4l)
- **Purpose:** the canonical projection every downstream consumer of tenant rows reads — the analysis, the Rent Roll and its CSV, the v1 Decision Report, Ask AI, conversion. One row per leasehold from the resolved terms (`AcquisitionTerms.resolveFamilyTerms` over that family's documents and decisions), then any raw `review.data.tenants[]` row nothing represents, marked `unfiled` and never verified. See `docs/ACQUISITION_REVIEW.md` §4l.
- **Inputs/Outputs:** `leaseholdRows({ families, documents (with abstracted_fields), decisions, tenants }, { terms, reasoner })` → `{ rows, leaseholds, unfiled, dropped[], resolverAvailable }`; `tenantRowFor(family, resolved)`; `legacyRows(tenants, documents, families)`; `canonicalValue(term)` (a contested term has no value); `attachStates(tenantSummary, rows)`; `cellState(row, field)` → `unverified · contested · missing · entered · verified · read`; `SOURCE`, `UNFILED_NOTE`, `ENTERED_NOTE` (held equal to `AcquisitionTerms.ENTERED_NOTE` by test); Option B (§4n): `analysisRows(projection)` (the leaseholds only — what the analysis and conversion may use), `unmatchedEntries(projection, resolutions, families)` → `[{ row, key, why, resolution }]`, `unresolvedCount(…)`, `RESOLUTION` (`matched` · `new_leasehold` · `dismissed`). Documents: `AcquisitionDocuments.pendingDocuments(documents, families, dispositions)` and `DISPOSITION` (`not_relevant` · `duplicate`) decide which source documents still wait for a person; consumers of the stored analysis read `_acqReviewsForConsumers()` in `script.js`. Pure; no DOM, no network. Loaded after the report view, before `script.js`.
- **Rules it keeps:** `review.data.tenants[]` is never modified or merged; a raw row is projected only when nothing else represents it, as a copy, with no states; a leasehold row's value is the resolved value, null for contested or missing; an entered value carries `_origins[field] = 'entered'`; `quotes` carries each governing clause so the engine's findings keep their citations, and none for an entered value.
- **Wired by:** `_acqCanonicalRows` / `_acqBuildAnalysis` / `_acqRefreshAnalysis` / `_acqConversionReview` in `script.js` (the last three through `_acqLeaseholdsOnly`); the gate `_acqConversionBlock`; the resolutions `acqResolveExtraction` / `acqReopenExtraction` and their list `_acqUnfiledListHtml`; the Enter control (`acqEnterTerm`) writes an entered correction through `AcquisitionTerms.buildDecisionPayload(…, { entered: true })`.
- **Tests:** `test-acquisition-leasehold.js`, `test-e2e-acquisition-canonical.js`, `tools/acquisition-canonical-mutation.js`; the entry path in `test-acquisition-decisions.js`; Option B: `test-acquisition-leaseholds-only.js`, `test-e2e-acquisition-leaseholds-only.js`, `tools/acquisition-leaseholds-mutation.js`.


## acquisition-lease-matrix.js — the Lease Matrix and a leasehold's record head (§4m, prototype)
- **Purpose:** the review's entry point. One matrix row per canonical leasehold (tenant, leased SF, base rent, expiration, lease type · CAM cap, status), the unfiled rows listed apart, and what one leasehold's record opens with. See `docs/ACQUISITION_REVIEW.md` §4m.
- **Inputs/Outputs:** `buildMatrix(canonicalRows, { terms })` → `{ leaseholds[], unfiled[] }`; `leaseholdEntry(row)`; `cellFor(row, field)` → `verified · entered · read · unclear · contested · missing`; `attentionFor(row)` (the issues behind a row's status: every contested term, then a key term missing or unclear); `attentionSummary(row)` (Needs attention as the workload: every term not verified, grouped by state — contested, not established, unclear, read by AI — each with its count and its terms); `statusFor(row)` → `issues · missing · unclear · unverified · verified`; `structureFor(row)`; `headline(entry)`; `formatValue(value, type)`; `KEY_FIELDS`; for the record: `DETAIL_GROUPS` / `DETAIL_ORDER` / `detailOrder(fields)` (lease-review order, every field once), `termRows(resolvedTerms)` (the Lease terms layer, grouped, each group tagged `core` or `other`), `termSections(resolvedTerms)` (Core lease terms, then Other lease terms, each with its groups and its counts by state), `sectionLine(section)`, `CORE_FIELDS`, `TIERS`, `contestedReadings(term)` (each document's own reading of a contested term, file-name order), `termState(term)`; for the header: `summaryLine(matrix, { documents })` (leasehold readiness, never a count of every term, then the documents still to match); the five states' words: `STATE_LABEL`, `TERM_STATE_LABEL`, `countsLine(counts)`, `summaryCounts(resolverSummary)`, `documentsLine({ unmatched, untyped })`; `unverifiedFor(row)` (values only read by AI, in review order — each entry's `unverified`). Pure; no DOM, no network. Loaded after `acquisition-leasehold.js`.
- **Rules it keeps:** reads only the §4l rows and their `_states` / `_origins` — resolves nothing, never reads `review.data.tenants[]`, adds no field or state; a contested term has no value ("Contested"); a missing one is never blank, zero or "none"; an entered value is marked entered; an unfiled row gets no status and no record.
- **Wired by:** `_renderAcqTerms` in `script.js` draws the matrix (`_acqLeaseMatrixHtml`) or the open leasehold's record (`_acqLeaseholdHeadHtml` + that leasehold's Lease Terms group) into `#acqTermsList`; `acqOpenLeasehold` / `acqBackToLeaseMatrix`; `_acqOpenLeasehold` is reset whenever a review is opened or closed.
- **Tests:** `test-acquisition-lease-matrix.js`, `test-e2e-acquisition-lease-matrix.js` (Maple Plaza, `fixtures/maple-plaza-acquisition.js`).
## Reports
