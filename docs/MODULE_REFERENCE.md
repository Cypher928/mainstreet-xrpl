...
- **Wired by:** `_acqCanonicalRows` / `_acqBuildAnalysis` / `_acqRefreshAnalysis` / `_acqConversionReview` in `script.js`; the Enter control (`acqEnterTerm`) writes an entered correction through `AcquisitionTerms.buildDecisionPayload(…, { entered: true })`.
- **Tests:** `test-acquisition-leasehold.js`, `test-e2e-acquisition-canonical.js`, `tools/acquisition-canonical-mutation.js`; the entry path in `test-acquisition-decisions.js`.
