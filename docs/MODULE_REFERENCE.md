...
- **Inputs/Outputs:** `buildMatrix(canonicalRows, { terms })` → `{ leaseholds[], unfiled[] }`; `leaseholdEntry(row)`; `cellFor(row, field)` → `verified · entered · read · unclear · contested · missing`; `attentionFor(row)` (every contested term, then a key term missing or unclear); `statusFor(row)` → `issues · missing · unclear · unverified · verified`; `structureFor(row)`; `headline(entry)`; `attentionHeading(entry)`; `formatValue(value, type)`; `KEY_FIELDS`; for the record: `DETAIL_GROUPS` / `DETAIL_ORDER` / `detailOrder(fields)` (lease-review order, every field once), `termRows(resolvedTerms)` (the Lease terms layer, grouped, each group tagged `core` or `other`), `termSections(resolvedTerms)` (Core lease terms, then Other lease terms, each with its groups and its counts by state), `sectionLine(section)`, `CORE_FIELDS`, `TIERS`, `contestedReadings(term)` (each document's own reading of a contested term, file-name order), `termState(term)`; for the header: `summaryLine(matrix)` (leasehold readiness, never a count of every term). Pure; no DOM, no network. Loaded after `acquisition-leasehold.js`.
