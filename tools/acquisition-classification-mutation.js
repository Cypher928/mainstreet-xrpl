...
const MUTANTS = [
  ...
  // ── P4-3 remediation, Issue B: a leasehold can begin ─────────────────────
  // Everything here is a rule about NOT guessing, so every mutant makes it
  // guess — and one (B02) simply puts the Pilot's bug back.
  { id: 'B01', file: D, why: "the AI path gains the human's power to begin a leasehold",
    from: "    if (type === 'original_lease') {\n      return { kind: 'new', label: _str(d.tenant_hint || d.tenantHint, 300),",
    to:   "    if (true) {\n      return { kind: 'new', label: _str(d.tenant_hint || d.tenantHint, 300)," },
  { id: 'B02', file: S, why: "THE PILOT'S BUG, RESTORED: a human correction files nothing",
    from: '  if (AD.isFamilyType(nextType) && !row.family_id) {',
    to:   '  if (false) {' },
  { id: 'B03', file: D, why: 'a tenant is read from the file name when nothing else names one',
    from: '    return read || _str(d.tenant_hint || d.tenantHint, 300);',
    to:   '    return read || _str(d.tenant_hint || d.tenantHint, 300) || _str(d.file_name, 300);' },
  { id: 'B04', file: D, why: 'any lease-family type begins a leasehold, with no person asking',
    from: "    if (type === 'original_lease' || o.beginLeasehold === true) {",
    to:   '    if (true) {' },
  { id: 'B05', file: D, why: 'a match is filed as CONFIRMED — a machine reading presented as a decision',
    from: "      return { kind: 'existing', familyId: matches[0].id,\n               status: 'proposed', source: 'ai',",
    to:   "      return { kind: 'existing', familyId: matches[0].id,\n               status: 'confirmed', source: 'human'," },
  { id: 'B06', file: D, why: 'an ambiguous tenant is offered to a person as a NEW leasehold, hiding the ambiguity',
    from: "      return { kind: 'none',\n               reason: 'More than one leasehold names this tenant — which one is a decision for a person.' };",
    to:   "      return { kind: 'none', canBegin: true,\n               reason: 'More than one leasehold names this tenant — which one is a decision for a person.' };" },
  { id: 'B07', file: D, why: 'a document with no tenant read is filed anyway',
    from: "      return { kind: 'none',\n               reason: 'No tenant has been read from this document yet, so which leasehold it belongs to is not known.' };",
    to:   "      return { kind: 'new', status: 'confirmed', source: 'human', label: 'Unknown tenant', tenantHint: null };" },
  { id: 'B08', file: D, why: 'siblings already filed elsewhere are swept into the new leasehold',
    from: '        && !r.family_id\n        && isFamilyType(r.doc_type)',
    to:   '        && isFamilyType(r.doc_type)' },
  { id: 'B09', file: D, why: 'a superseded upload is swept in with the rest',
    from: '        && isCurrent(r)\n        && normalizeParty(tenantHintFor(r) || \'\') === party;',
    to:   "        && normalizeParty(tenantHintFor(r) || '') === party;" },
  { id: 'B10', file: D, why: 'a rent roll is swept into a leasehold by tenant name',
    from: '        && isFamilyType(r.doc_type)\n        && isCurrent(r)',
    to:   '        && isCurrent(r)' },
  { id: 'B11', file: S, why: 'the offered siblings are CONFIRMED in bulk instead of one at a time',
    from: "      familyId, familyStatus: 'proposed', familySource: 'ai',\n      classificationHistory: AD.appendHistory(s.classification_history, {",
    to:   "      familyId, familyStatus: 'confirmed', familySource: 'human',\n      classificationHistory: AD.appendHistory(s.classification_history, {" },
  { id: 'B12', file: S, why: 'a new leasehold is filed as a proposal — the human act is lost',
    from: "      familyId: created.id, familyStatus: 'confirmed', familySource: 'human',",
    to:   "      familyId: created.id, familyStatus: 'proposed', familySource: 'ai'," },
  { id: 'B13', file: S, why: 'the begin control is reachable for a document already filed',
    from: '  if (!row || !AD.isFamilyType(row.doc_type) || row.family_id) return;',
    to:   '  if (!row || !AD.isFamilyType(row.doc_type)) return;' },
  { id: 'B14', file: S, why: 'the begin control names nobody — 024 would refuse it, silently here',
    from: '    confirmedBy: user.id, confirmedAt: new Date().toISOString(),\n    classificationHistory: AD.appendHistory(row.classification_history, step.patch.historyEntry),',
    to:   '    classificationHistory: AD.appendHistory(row.classification_history, step.patch.historyEntry),' },
  { id: 'B15', file: S, why: 'the history is REPLACED rather than appended — the audit trail is lost',
    from: '    classificationHistory: AD.appendHistory(row.classification_history, step.patch.historyEntry),',
    to:   '    classificationHistory: [step.patch.historyEntry],' },
  { id: 'B16', file: S, why: 'the D-17 clearing branch also runs on the way IN',
    from: '  if (!AD.isFamilyType(nextType) && row.family_id) {',
    to:   '  if (row.family_id) {' },
  { id: 'B17', file: S, why: 'the correction re-files a document somebody already placed',
    from: '  if (AD.isFamilyType(nextType) && !row.family_id) {',
    to:   '  if (AD.isFamilyType(nextType)) {' },
  { id: 'B18', file: S, why: 'the panel stops saying why a document has no leasehold',
    from: "      if (v.kind === 'none') {\n        unfiled = '<div class=\"acq-doc-unfiled\">'",
    to:   "      if (false) {\n        unfiled = '<div class=\"acq-doc-unfiled\">'" },
  { id: 'B19', file: S, why: 'the control is offered wherever a document is unfiled, ambiguity and all',
    from: '          + (v.canBegin',
    to:   '          + (true' },
  { id: 'B20', file: S, why: 'the screen claims MainStreet identified the original lease',
    from: "              ? '<div class=\"acq-doc-unfiled-why\">No matching leasehold found. '",
    to:   "              ? '<div class=\"acq-doc-unfiled-why\">This document was identified as the original lease. '" },
...
const SUITES = [
  ['node', 'test-acquisition-family-lifecycle.js'],
