...
const CLASSIFICATION = {
  ...
  recordVacantSpace: { kind: 'browser_only', why: 'a WRITE path — records a space as vacant on the open property (tenantData, property.tenants, savePropertyData) and must never be reachable from a read-only server module. tenant-space.js reads it only inside submitVacantForm, the Spaces list\'s "Mark space vacant" form handler, never from assemble(); the call site guards it with typeof window.recordVacantSpace === "function", and absence is the handled branch (the form reports that a vacancy cannot be recorded on this page)' },
  ...
  varianceBreakdownOnScreen: { kind: 'browser_only', why: 'the variance breakdown behind the banner currently on screen, scoped to the property it was rendered for. property-workspace.js reads it so the Overview\'s unbilled-pool attention item describes that gap with the causes the CAM panel already named — a lease cap withheld, a coverage shortfall, a lease exclusion — instead of subtracting and calling the total "unrecovered underbilling". It closes over the browser\'s render-time globals (_lastVarianceBreakdown and the scope stamped beside it), so it has no meaning on a server, and absence is the handled branch: _scopedVarianceBreakdown returns null and the item states only that the money was not billed, leaving the explanation to the surface that has one. Re-deriving it server-side is not an option either — the snapshot it would have to use lacks the invoice dates and match fields, and reports a residual the run did not produce' },
  ...
  // Property Workspace V2. The cabinet MAP is a pure module with a
  // module.exports, but no server module requires it: tenant-space.js reads
  // window.PropertyCabinet only inside _isVacantRow (the Spaces LIST, never
  // assemble()) and carries the same strict-boolean fallback inline, so its
  // absence on the server changes nothing. The cabinet VIEW renders the
  // Property tab; property-os.js and property-workspace.js reach for it only
  // to open a drawer from a click, and every read is guarded.
  PropertyCabinet:     { kind: 'browser_only', why: 'read by tenant-space.js only in the Spaces list (isVacant), with the identical strict-boolean fallback inline; assemble() never reaches it, so the server needs nothing supplied' },
  PropertyCabinetView: { kind: 'browser_only', why: 'the Property tab renderer (drawers, addressing); property-workspace.js reads it only from act(), a click handler, and guards it with if (window.PropertyCabinetView && ...openDrawer)' },
