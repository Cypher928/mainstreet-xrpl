...
const MUTANTS = [
  ...
  { id: 'S11', file: S, why: 'a pre-§4l analysis is never flagged as behind the terms',
    from: "  if (!a.canonical) return [{ key: 'basis', text: 'This analysis was run from the uploaded files, before the Rent Roll read the lease terms.',",
    to:   "  if (!a.canonical) return []; if (0) return [{ key: 'basis', text: 'This analysis was run from the uploaded files, before the Rent Roll read the lease terms.'," },
